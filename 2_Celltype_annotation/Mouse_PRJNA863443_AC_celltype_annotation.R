...
load("/Volumes/OIProject_II/1_R/2_Integration/Mouse/Benayoun_lab_Aging_model/2025-06-23_10x_ovary_Mouse_Benayoun_lab_Aging_integrated_harmony.RData")
...
#scGate it!
my.ovary.AC.Ptprc.pos <- scGate(data = ovary.Mouse.AC.harmony, model = my.scGate.model.Ptprc, assay = "RNA", verbose = TRUE)

t.pure <- as.matrix(table(my.ovary.AC.Ptprc.pos$is.pure, my.ovary.AC.Ptprc.pos$seurat_clusters))
...
# 14 15 17 20 21 27 30  5  6  7 

# Assign cell type to each seurat cluster: Ptprc (CD45) positive cells clusters: 14, 15, 17, 20, 21, 27, 30, 5, 6, 7

my.ovary.AC.Ptprc.pos <- SetIdent(my.ovary.AC.Ptprc.pos, value = "seurat_clusters")
ovary.AC <- my.ovary.AC.Ptprc.pos

ovary.AC@meta.data$celltype.level1 <- rep("Ptprc.neg",dim(ovary.AC@meta.data)[1])
ovary.AC@meta.data$celltype.level1[ovary.AC@meta.data$seurat_clusters %in% high_purity_cluster_numbers] <- "Ptprc.pos"

save(ovary.AC, file = paste0(Sys.Date(),"_10x_ovary_Benayoun_AC_Ptprc_gated_Seurat_object.RData"))
...
ovary.AC.Ptprc.pos <- subset(ovary.AC, subset = celltype.level1 == "Ptprc.pos")

DefaultAssay(ovary.AC.Ptprc.pos) <- "RNA"

########## Normalize, SCT, dimreduc data ##########

ovary.AC.Ptprc.pos <- NormalizeData(ovary.AC.Ptprc.pos, normalization.method = "LogNormalize", scale.factor = 10000)
ovary.AC.Ptprc.pos <- SCTransform(object = ovary.AC.Ptprc.pos, vars.to.regress = c("nFeature_RNA", "percent.mito", "decontX_contamination", "Library"))

save(ovary.AC.Ptprc.pos, file = paste0(Sys.Date(),"_10x_ovary_AC_Ptprc_pos_cells_Seurat_object_clean_postSCT.RData"))

ovary.AC.Ptprc.pos <- RunPCA(ovary.AC.Ptprc.pos, npcs = 50)

# Determine the ‘dimensionality’ of the dataset
pdf(paste0(Sys.Date(), "_10x_ovary_AC_Ptprc_pos_cells_ElbowPlot.pdf"))
ElbowPlot(ovary.AC.Ptprc.pos, ndims = 50)
dev.off()
...
pct <- ovary.AC.Ptprc.pos[["pca"]]@stdev / sum(ovary.AC.Ptprc.pos[["pca"]]@stdev) * 100
...
ovary.AC.Ptprc.pos <- RunUMAP(ovary.AC.Ptprc.pos, dims = 1:pcs)
ovary.AC.Ptprc.pos <- FindNeighbors(ovary.AC.Ptprc.pos, dims = 1:pcs)
ovary.AC.Ptprc.pos <- FindClusters(object = ovary.AC.Ptprc.pos)
ovary.AC.Ptprc.pos <- FindClusters(object = ovary.AC.Ptprc.pos, resolution = 3)
...
my.SingleCellExperiment.object <- as.SingleCellExperiment(ovary.AC.Ptprc.pos)
...
my.singler.immgen$meta.data$Library       =    ovary.AC.Ptprc.pos@meta.data$Library                       # Library IDs

save(my.singler.immgen, file = paste(Sys.Date(),"10x_ovary_Benayoun_lab_AC_Ptprc_pos_cells_SingleR_object_Immgen.RData",sep = "_"))
...
ovary.AC.Ptprc.pos[["SingleR.Immgen"]] <- my.singler.immgen$labels

###########################
# Marker gene expression plots

DefaultAssay(ovary.AC.Ptprc.pos) <- "SCT"

table.SingleR.annotation <- table(ovary.AC.Ptprc.pos@meta.data$seurat_clusters, ovary.AC.Ptprc.pos@meta.data$SingleR.Immgen)
...
my.immune.marker.list$Neutrophils <- intersect(rownames(ovary.AC.Ptprc.pos), my.immune.marker.list$Neutrophils)
my.immune.marker.list$Macrophages <- intersect(rownames(ovary.AC.Ptprc.pos), my.immune.marker.list$Macrophages)
my.immune.marker.list$Monocytes <- intersect(rownames(ovary.AC.Ptprc.pos), my.immune.marker.list$Monocytes)
my.immune.marker.list$DC <- intersect(rownames(ovary.AC.Ptprc.pos), my.immune.marker.list$DC)
my.immune.marker.list$'NK cells' <- intersect(rownames(ovary.AC.Ptprc.pos), my.immune.marker.list$'NK cells')
my.immune.marker.list$ILC <- intersect(rownames(ovary.AC.Ptprc.pos), my.immune.marker.list$ILC)
my.immune.marker.list$NKT <- intersect(rownames(ovary.AC.Ptprc.pos), my.immune.marker.list$NKT)
my.immune.marker.list$'T cells' <- intersect(rownames(ovary.AC.Ptprc.pos), my.immune.marker.list$'T cells')
my.immune.marker.list$'B cells' <- intersect(rownames(ovary.AC.Ptprc.pos), my.immune.marker.list$'B cells')
...
topgenes <- head(VariableFeatures(ovary.AC.Ptprc.pos), 2500)
expr <- GetAssayData(ovary.AC.Ptprc.pos)
...
ovary.AC.Ptprc.pos[["scSorter.Immgen"]] <- rts$Pred_Type

table.scSorter.annotation <- table(ovary.AC.Ptprc.pos@meta.data$seurat_clusters, ovary.AC.Ptprc.pos@meta.data$scSorter.Immgen)
...
es.max  = sctype_score(scRNAseqData = ovary.AC.Ptprc.pos[["SCT"]]@scale.data, scaled = TRUE, gs = gs_list$gs_positive, gs2 = gs_list$gs_negative)

cL_resutls = do.call("rbind", lapply(unique(ovary.AC.Ptprc.pos@meta.data$SCT_snn_res.2), function(cl){
  es.max.cl = sort(rowSums(es.max[ ,rownames(ovary.AC.Ptprc.pos@meta.data[ovary.AC.Ptprc.pos@meta.data$SCT_snn_res.2==cl, ])]), decreasing = !0)
  head(data.frame(cluster = cl, type = names(es.max.cl), scores = es.max.cl, ncells = sum(ovary.AC.Ptprc.pos@meta.data$SCT_snn_res.2==cl)), 10)
}))
...
ovary.AC.Ptprc.pos@meta.data$scType_raw = ""

for(i in 1:ncol(es.max)){
  ovary.AC.Ptprc.pos@meta.data$scType_raw[rownames(ovary.AC.Ptprc.pos@meta.data) %in% colnames(es.max)[i] ] = as.character(rownames(es.max)[which.max(es.max[,i])])
}

table.scType.annotation <- table(ovary.AC.Ptprc.pos@meta.data$seurat_clusters, ovary.AC.Ptprc.pos@meta.data$scType_raw)

save(ovary.AC.Ptprc.pos, file = paste(Sys.Date(),"10x_ovary_Benayoun_lab_AC_Ptprc_positive_Seurat_object_with_cell_type_annotations_intermediate.RData",sep = "_"))
...
save(annotation.results, file = paste(Sys.Date(),"10x_ovary_Benayoun_lab_AC_Ptprc_pos_cells_annotation_results.RData",sep = "_"))
write.table(annotation.results, file = paste(Sys.Date(),"10x_ovary_Benayoun_lab_AC_Ptprc_pos_cells_annotation_results.txt",sep = "_"), quote = FALSE)

# Manual marker gene analysis

ovary.AC.Ptprc.pos <- SetIdent(ovary.AC.Ptprc.pos, value = "seurat_clusters")

VlnPlot(subset(ovary.AC.Ptprc.pos, subset = seurat_clusters %in% c(0:7)), 
...
VlnPlot(subset(ovary.AC.Ptprc.pos, subset = seurat_clusters %in% c(8:14)), 
        features = c("S100a8",                                        # S100a8 - Neutrophil marker
                     "Ly6c2", "C1qa",                                 # Monocyte (Cd68, Ly6c2) & Macrophage (Cd63, C1qa) markers
                     "Flt3",                                          # DC marker
                     "Il2ra", "Ccdc184", "Klrb1c", "Fam184b", # ILC
...
VlnPlot(subset(ovary.AC.Ptprc.pos, subset = seurat_clusters %in% c(15:21)), 
        features = c("S100a8",                                        # S100a8 - Neutrophil marker
                     "Ly6c2", "C1qa",                                 # Monocyte (Cd68, Ly6c2) & Macrophage (Cd63, C1qa) markers
                     "Flt3",                                          # DC marker
                     "Il2ra", "Ccdc184", "Klrb1c", "Fam184b", # ILC
...
VlnPlot(subset(ovary.AC.Ptprc.pos, subset = seurat_clusters %in% c(22:26)), 
        features = c("S100a8",                                        # S100a8 - Neutrophil marker
                     "Ly6c2", "C1qa",                                 # Monocyte (Cd68, Ly6c2) & Macrophage (Cd63, C1qa) markers
                     "Flt3",                                          # DC marker
                     "Il2ra", "Ccdc184", "Klrb1c", "Fam184b", # ILC
...
# Refer to ./2024-10-15_10x_ovary_Benayoun_lab_AC_Ptprc_positive_cells_annotation_results_FINAL.txt
################################

ovary.AC.Ptprc.pos@meta.data$celltype.level2 <- rep("NA",dim(ovary.AC.Ptprc.pos@meta.data)[1])
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 0] <- "CD8NKT"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 1] <- "DNT"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 2] <- "DNT"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 3] <- "CD8NKT"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 4] <- "NK"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 5] <- "DNT"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 6] <- "NK"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 7] <- "CD4T"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 8] <- "B"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 9] <- "CD8NKT"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 10] <- "CD8NKT"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 11] <- "DNT"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 12] <- "NK"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 13] <- "CD4T"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 14] <- "Myeloid"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 15] <- "NK"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 16] <- "B"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 17] <- "Myeloid"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 18] <- "B"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 19] <- "DC"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 20] <- "DNT"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 21] <- "DNT"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 22] <- "ILC"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 23] <- "DNT"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 24] <- "CD8NKT"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 25] <- "DC"
ovary.AC.Ptprc.pos@meta.data$celltype.level2[ovary.AC.Ptprc.pos@meta.data$seurat_clusters %in% 26] <- "Neutrophil"

save(ovary.AC.Ptprc.pos, file = paste(Sys.Date(),"10x_ovary_Benayoun_lab_AC_Ptprc_positive_cells_Seurat_object_with_final_annotation.RData",sep = "_"))
...
ovary.AC.Ptprc.neg <- subset(ovary.AC, subset = celltype.level1 == "Ptprc.neg")

DefaultAssay(ovary.AC.Ptprc.pos) <- "RNA"

########## Normalize, SCT, dimreduc data ##########

ovary.AC.Ptprc.neg <- NormalizeData(ovary.AC.Ptprc.neg, normalization.method = "LogNormalize", scale.factor = 10000)
ovary.AC.Ptprc.neg <- SCTransform(object = ovary.AC.Ptprc.neg, vars.to.regress = c("nFeature_RNA", "percent.mito", "decontX_contamination", "Library"))

save(ovary.AC.Ptprc.neg, file = paste0(Sys.Date(),"_10x_ovary_AC_Ptprc_neg_cells_Seurat_object_clean_postSCT.RData"))

ovary.AC.Ptprc.neg <- RunPCA(ovary.AC.Ptprc.neg, npcs = 50)

# Determine the ‘dimensionality’ of the dataset
pdf(paste0(Sys.Date(), "_10x_ovary_AC_Ptprc_neg_cells_ElbowPlot.pdf"))
ElbowPlot(ovary.AC.Ptprc.neg, ndims = 50)
dev.off()
...
# Determine percent of variation associated with each PC
pct <- ovary.AC.Ptprc.neg[["pca"]]@stdev / sum(ovary.AC.Ptprc.neg[["pca"]]@stdev) * 100
...
###############################################################################
# run dimensionality reduction algorithm
ovary.AC.Ptprc.neg <- RunUMAP(ovary.AC.Ptprc.neg, dims = 1:pcs)
ovary.AC.Ptprc.neg <- FindNeighbors(ovary.AC.Ptprc.neg, dims = 1:pcs)
ovary.AC.Ptprc.neg <- FindClusters(object = ovary.AC.Ptprc.neg)
ovary.AC.Ptprc.neg <- FindClusters(object = ovary.AC.Ptprc.neg, resolution = 3)
...
# Prep test data
DefaultAssay(ovary.AC.Ptprc.neg) <- "RNA"
my.SingleCellExperiment.object <- as.SingleCellExperiment(ovary.AC.Ptprc.neg)
...
ovary.AC.Ptprc.neg[["SingleR.cycling"]] <- my.singleR.cycling$labels

###########################
# Marker gene expression plots

DefaultAssay(ovary.AC.Ptprc.neg) <- "SCT"

ovary.AC.Ptprc.neg <- SetIdent(ovary.AC.Ptprc.neg, value = "SingleR.cycling")
levels(ovary.AC.Ptprc.neg) <- c("Oocyte",
                                "Granulosa",
                                "Theca",
                                "Stroma",
                                "Mesenchyme",
                                ...
                                "LEC",
                                "Epithelial")
DotPlot(ovary.AC.Ptprc.neg, features = c("Ddx4",                             # Oocyte marker
...
DotPlot(ovary.AC.Ptprc.neg, features = c("Amh", "Fshr","Foxl2", "Nr5a2", "Kitl", "Cyp19a1",     # Granulosa cell marker
...
# table(ovary.AC.Ptprc.neg@meta.data$seurat_clusters, ovary.AC.Ptprc.neg@meta.data$SingleR.cycling)

table.SingleR.cycling.annotation <- table(ovary.AC.Ptprc.neg@meta.data$seurat_clusters, ovary.AC.Ptprc.neg@meta.data$SingleR.cycling)
...
# Transfer cell type annotation to Seurat
ovary.AC.Ptprc.neg[["SingleR.GSE232309"]] <- my.singleR.GSE232309$labels
...
DimPlot(ovary.AC.Ptprc.neg, reduction = "umap", group.by = "SingleR.GSE232309")
dev.off()
...
ovary.AC.Ptprc.neg <- SetIdent(ovary.AC.Ptprc.neg, value = "SingleR.GSE232309")
levels(ovary.AC.Ptprc.neg) <- c("Oocyte",
                                "Granulosa",
                                "Theca",
                                "Stroma",
...
table.SingleR.GSE232309.annotation <- table(ovary.AC.Ptprc.neg@meta.data$seurat_clusters, ovary.AC.Ptprc.neg@meta.data$SingleR.GSE232309)
...
my.ovarian.marker.list$Oocyte <- intersect(rownames(ovary.AC.Ptprc.neg), my.ovarian.marker.list$Oocyte)
my.ovarian.marker.list$Granulosa <- intersect(rownames(ovary.AC.Ptprc.neg), my.ovarian.marker.list$Granulosa)
my.ovarian.marker.list$Theca <- intersect(rownames(ovary.AC.Ptprc.neg), my.ovarian.marker.list$Theca)
my.ovarian.marker.list$Stroma <- intersect(rownames(ovary.AC.Ptprc.neg), my.ovarian.marker.list$Stroma)
my.ovarian.marker.list$Mesenchyme <- intersect(rownames(ovary.AC.Ptprc.neg), my.ovarian.marker.list$Mesenchyme)
my.ovarian.marker.list$BEC <- intersect(rownames(ovary.AC.Ptprc.neg), my.ovarian.marker.list$BEC)
my.ovarian.marker.list$LEC <- intersect(rownames(ovary.AC.Ptprc.neg), my.ovarian.marker.list$LEC)
my.ovarian.marker.list$Epithelial <- intersect(rownames(ovary.AC.Ptprc.neg), my.ovarian.marker.list$Epithelial)
...
# Pre-process data for scSorter
topgenes <- head(VariableFeatures(ovary.AC.Ptprc.neg), 2500)
expr <- GetAssayData(ovary.AC.Ptprc.neg)
topgene_filter <- rowSums(as.matrix(expr)[topgenes, ]!=0) > ncol(expr)*.05
topgenes <- topgenes[topgene_filter]
...
# Transfer scSorter annotations to Seurat Object
ovary.AC.Ptprc.neg[["scSorter.labels"]] <- rts$Pred_Type

# table(ovary.AC.Ptprc.neg@meta.data$seurat_clusters, ovary.AC.Ptprc.neg@meta.data$scSorter.labels)

table.scSorter.annotation <- table(ovary.AC.Ptprc.neg@meta.data$seurat_clusters, ovary.AC.Ptprc.neg@meta.data$scSorter.labels)
...
# Finally, let's assign cell types to each cluster:
# get cell-type by cell matrix
es.max  = sctype_score(scRNAseqData = ovary.AC.Ptprc.neg[["SCT"]]@scale.data, scaled = TRUE, gs = gs_list$gs_positive, gs2 = gs_list$gs_negative)

cL_resutls = do.call("rbind", lapply(unique(ovary.AC.Ptprc.neg@meta.data$seurat_clusters), function(cl){
  es.max.cl = sort(rowSums(es.max[ ,rownames(ovary.AC.Ptprc.neg@meta.data[ovary.AC.Ptprc.neg@meta.data$seurat_clusters==cl, ])]), decreasing = !0)
  head(data.frame(cluster = cl, type = names(es.max.cl), scores = es.max.cl, ncells = sum(ovary.AC.Ptprc.neg@meta.data$seurat_clusters==cl)), 10)
}))
...
ovary.AC.Ptprc.neg@meta.data$scType_raw = ""

for(i in 1:ncol(es.max)){
  ovary.AC.Ptprc.neg@meta.data$scType_raw[rownames(ovary.AC.Ptprc.neg@meta.data) %in% colnames(es.max)[i] ] = as.character(rownames(es.max)[which.max(es.max[,i])])
}

table.scType.annotation <- table(ovary.AC.Ptprc.neg@meta.data$seurat_clusters, ovary.AC.Ptprc.neg@meta.data$scType_raw)

save(ovary.AC.Ptprc.neg, file = paste(Sys.Date(),"10x_ovary_Benayoun_lab_AC_Ptprc_negativetive_Seurat_object_with_cell_type_annotations_intermediate.RData",sep = "_"))
...
save(annotation.results, file = paste(Sys.Date(),"10x_ovary_Benayoun_lab_AC_Ptprc_negative_cells_annotation_results.RData",sep = "_"))
write.table(annotation.results, file = paste(Sys.Date(),"10x_ovary_Benayoun_lab_AC_Ptprc_negative_cells_annotation_results.txt",sep = "_"), quote = FALSE)
...
VlnPlot(subset(ovary.AC.Ptprc.neg, subset = seurat_clusters %in% c(0:7)), 
...
VlnPlot(subset(ovary.AC.Ptprc.neg, subset = seurat_clusters %in% c(8:16)), 
        features = c("Ddx4",                             # Oocyte marker
                     "Amh", "Foxl2", "Cyp19a1",          # Granulosa cell marker
                     "Cyp17a1", "Cyp11a1", "Lhcgr",      # Theca cell marker 
                     "Pdgfra", "Col1a1",                 # Stroma cell marker
...
VlnPlot(subset(ovary.AC.Ptprc.neg, subset = seurat_clusters %in% c(17:24)), 
        features = c("Ddx4",                             # Oocyte marker
                     "Amh", "Foxl2", "Cyp19a1",          # Granulosa cell marker
                     "Cyp17a1", "Cyp11a1", "Lhcgr",      # Theca cell marker 
                     "Pdgfra", "Col1a1",                 # Stroma cell marker
...
VlnPlot(subset(ovary.AC.Ptprc.neg, subset = seurat_clusters %in% c(24:32)), 
        features = c("Ddx4",                             # Oocyte marker
                     "Amh", "Foxl2", "Cyp19a1",          # Granulosa cell marker
                     "Cyp17a1", "Cyp11a1", "Lhcgr",      # Theca cell marker 
                     "Pdgfra", "Col1a1",                 # Stroma cell marker
...
################################
# Transfer final annotations
# Refer to ./2024-10-15_10x_ovary_Benayoun_lab_AC_Ptprc_negative_cells_annotation_results_FINAL.txt
################################

ovary.AC.Ptprc.neg <- SetIdent(ovary.AC.Ptprc.neg, value = "seurat_clusters")

VlnPlot(ovary.AC.Ptprc.neg, features = c("nCount_RNA"))
VlnPlot(ovary.AC.Ptprc.neg, features = c("nFeature_RNA"))
VlnPlot(ovary.AC.Ptprc.neg, features = c("percent.mito"))
VlnPlot(ovary.AC.Ptprc.neg, features = c("decontX_contamination"))

ovary.AC.Ptprc.neg@meta.data$celltype.level2 <- rep("NA",dim(ovary.AC.Ptprc.neg@meta.data)[1])
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 0] <- "Epithelial"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 1] <- "Theca"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 2] <- "Stroma"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 3] <- "Stroma"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 4] <- "BEC"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 5] <- "Stroma"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 6] <- "LEC"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 7] <- "Stroma"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 8] <- "Stroma"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 9] <- "Stroma"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 10] <- "Stroma"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 11] <- "Stroma"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 12] <- "Theca"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 13] <- "Stroma"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 14] <- "Stroma"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 15] <- "Granulosa"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 16] <- "LEC"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 17] <- "Stroma"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 18] <- "Stroma"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 19] <- "Stroma"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 20] <- "Granulosa"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 21] <- "Stroma"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 22] <- "Theca"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 23] <- "Granulosa"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 24] <- "BEC"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 25] <- "Granulosa"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 26] <- "Stroma"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 27] <- "SMC"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 28] <- "Stroma"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 29] <- "BEC"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 30] <- "BEC"
ovary.AC.Ptprc.neg@meta.data$celltype.level2[ovary.AC.Ptprc.neg@meta.data$seurat_clusters %in% 31] <- "Granulosa"

save(ovary.AC.Ptprc.neg, file = paste(Sys.Date(),"10x_ovary_Benayoun_lab_AC_Ptprc_negative_cells_Seurat_object_with_final_annotation.RData",sep = "_"))
...
celltype.level2.pos <- ovary.AC.Ptprc.pos@meta.data[, "celltype.level2", drop = FALSE]
celltype.level2.neg <- ovary.AC.Ptprc.neg@meta.data[, "celltype.level2", drop = FALSE]
...
common_cell_ids <- intersect(rownames(ovary.AC@meta.data), rownames(combined.celltype.level2))

# Update the celltype.level2 in ovary.AC for the matching cells
ovary.AC@meta.data[common_cell_ids, "celltype.level2"] <- combined.celltype.level2[common_cell_ids, "celltype.level2"]

# Verify that the metadata has been successfully updated
table(is.na(ovary.AC@meta.data$celltype.level2))
# FALSE 
# 9559 

save(ovary.AC, file = paste(Sys.Date(),"10x_ovary_Benayoun_lab_AC_Seurat_object_with_final_annotation.RData",sep = "_"))

################################################################################################################################################################
sink(file = paste(Sys.Date(),"_PRJNA863443_cell_type_annotation_session_Info.txt", sep =""))
sessionInfo()
sink()
